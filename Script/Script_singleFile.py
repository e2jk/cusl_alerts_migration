...
#Part 7 -- Formatting
df_transfusion['nte_3'] = df_transfusion['nte_3'].replace({'[aA]nti[-\s]':'Anti-'}, regex=True)

#Part 8 -- Filtering
to_transfer = ["Ac antiérythrocytaires",
...
for value in to_transfer:
    dict["df_transfusion_{0}".format(value.replace(' ','_'))] = df_transfusion[df_transfusion["al1_3"] == value]
...
for key,value in dict.items():
    ...
    #Create a new column with the lenght of the strings. This is used in the case where there are 2 different alerts on the same day, we want to retrieve the most complete.
    ...
    value.sort_values(["numpat", "al1_6", "length"], ascending = [True, False, False], inplace = True)
    value.drop_duplicates(["numpat","nte_3"], inplace=True, keep='last')

#Part 11 -- Try to aggregate and filter out duplicate. NOT VALIDATED !
dict_grouped_test={}
for key,value in dict.items():
    dict_grouped_test["grouped_{0}".format(key)] = dict[key].groupby('numpat')['nte_3'].apply(', '.join).reset_index(name='test_aggregate')
...
for key,value in dict_grouped_test.items():
    value["test"] = value["test_aggregate"].str.findall(r"((?:auto[ -])?[Aa]nti-[\s]*\S[^,. :;]*)")#(Anti-[\s]*\S[^,. :;]*)#(r"(Anti-\S[^, .:]*)")
...
for key,value in dict.items():
    ...
    dict_grouped["grouped_{0}".format(key)] = dict[key].groupby('numpat').apply(f).reset_index()

#Part 15 -- Adding the aggregate to the export format. NOT VALIDATED.
dict_grouped['grouped_df_transfusion_Ac_antiérythrocytaires']["aggregate (non validé)"] = dict_grouped_test['grouped_df_transfusion_Ac_antiérythrocytaires']["test_aggregate"]
dict_grouped['grouped_df_transfusion_Ac_antiérythrocytaires']["test (non validé)"] = dict_grouped_test['grouped_df_transfusion_Ac_antiérythrocytaires']["test"]
dict_grouped['grouped_df_transfusion_Ac_antiérythrocytaires']["test_dict (non validé)"] = dict_grouped_test['grouped_df_transfusion_Ac_antiérythrocytaires']["test_2"]

#Part 16 -- Export.
with pd.ExcelWriter(os.path.join(report_path,
                        'import_alert_transf' + ".xlsx")) as writer:
    for key in dict_grouped:
        # Par catégorie d'alerte, toutes les alertes regroupées de façon à n'avoir qu'une ligne par patient.
...
print("Attention, les colonnes \"aggregate\", \"test\" et \"test_2\" n'ont jamais été validées.")
print("Les données de ces colonnes ont été manipulées et modifiées et sont susceptibles d'être incorrectes.")
print("Ces colonnes n'ont pour vocation que de permettre détecter les patients n'ayant qu'un seul type d'anticorps, que l'on pourrait qualifier de \"bénin\"")